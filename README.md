...
1. **Compute refusal direction**: Identify the linear direction in activation space that distinguishes harmful from harmless prompt representations at a specific layer.

2. **Construct ablation targets**: For each training sample at the target layer:
   - *Allowed* samples: subtract the refusal direction projection (remove refusal signal)
   - *Disallowed/Safe* samples: add the refusal direction projection (reinforce refusal signal)

3. **Train LoRA**: Fit a LoRA adapter on the target layer to reproduce the ablated activations, effectively learning to selectively suppress refusal.

4. **Evaluate**: Verify that allowed categories are no longer refused while disallowed categories remain refused, and general capabilities (MMLU, GSM8K) are preserved.

## Citation

```bibtex
@article{palette2025,
  title={PALETTE: Per-Layer Ablation Training for Selective Safety Relaxation},
  author={},
  year={2025}
}
```
