...
from utils.dataset_utils import load_dataset, dump_image, MMMU_preproc
from utils.eval_utils import build_judge, eval_single_sample

# Set vLLM multiprocessing method
os.environ['VLLM_WORKER_MULTIPROC_METHOD'] = 'spawn'


def _check_vllm_model_compat(model_path: str):
    """
    Validate known compatibility constraints between model family and vLLM version.
    """
    model_name = str(model_path).lower()
    needs_qwen25_vl = ("qwen2.5-vl" in model_name)
    needs_qwen2_vl = ("qwen2-vl" in model_name) or needs_qwen25_vl

    if not needs_qwen2_vl:
        return

    vllm_version_raw = getattr(vllm, "__version__", "0.0.0")
    try:
        vllm_version = Version(vllm_version_raw)
    except InvalidVersion:
        return

    if vllm_version < Version("0.6.3"):
        raise RuntimeError(
            f"Incompatible vLLM version {vllm_version_raw} for {model_path}. "
            "Qwen2-VL requires vLLM >= 0.6.3."
        )

    if needs_qwen25_vl and vllm_version < Version("0.7.0"):
        raise RuntimeError(
            f"Incompatible vLLM version {vllm_version_raw} for {model_path}. "
            "Qwen2.5-VL needs a newer vLLM release with mRoPE support. "
            "Please upgrade vLLM to a recent stable version."
        )


def build_mmmu_prompt(line, dump_image_func, dataset, min_pixels, max_pixels):
...
def run_inference(args):
    ...
    _check_vllm_model_compat(args.model_path)
    try:
        llm = LLM(
        ...
        if "qwen2" in str(args.model_path).lower():
            msg = str(e) or "<empty>"
            raise RuntimeError(
                "Failed to initialize vLLM with a Qwen2-VL/Qwen2.5-VL model. "
                "This is usually caused by an old vLLM version that cannot parse "
                "the model rope/mrope config. Please upgrade vLLM "
                "(Qwen2-VL: >=0.6.3, Qwen2.5-VL: use a recent stable release). "
                f"Original AssertionError: {msg}"
            ) from e
        raise
...
def main():
    ...
    infer_parser.add_argument("--max-model-len", type=int, default=8192,
                              help="Maximum model context length (default: 8192, higher uses much more KV cache memory)")
    infer_parser.add_argument("--max-images-per-prompt", type=int, default=10,
                              help="Maximum images per prompt (default: 10)")
    infer_parser.add_argument("--min-pixels", type=int, default=1280 * 28 * 28,
                              help="Minimum image pixels for Qwen-VL smart resize")
    infer_parser.add_argument("--max-pixels", type=int, default=2560 * 28 * 28,
                              help="Maximum image pixels for Qwen-VL smart resize")
