...
def get_direction_ablation_input_pre_hook(direction: Tensor):
    def hook_fn(module, input):
        ...
        activation[:, -20:, :] -= (
                (activation[:, -20:, :] @ direction).unsqueeze(-1) * direction * 1.5
        )
        # activation -= (activation @ direction).unsqueeze(-1) * direction * 1.5
...
def get_direction_ablation_output_hook(direction: Tensor):
    def hook_fn(module, input, output):
        ...
        direction = direction / (direction.norm(dim=-1, keepdim=True) + 1e-8)
        direction = direction.to(activation)
        activation -= (activation @ direction).unsqueeze(-1) * direction * 2.5
